...
for ind1 in range(2,4):
    for ind2 in range(1,101):
        if(ind2<10):
            path = 'resampled-to-8bit/class-00'+str(ind1)+'-sample-00'+str(ind2)+'-8bit.png'
        elif(ind2<100):
            path = 'resampled-to-8bit/class-00'+str(ind1)+'-sample-0'+str(ind2)+'-8bit.png'
        else:
            path = 'resampled-to-8bit/class-00'+str(ind1)+'-sample-'+str(ind2)+'-8bit.png'
        print("Path:",path)
        ...
        # print(data.ndim) = 2.. hence a 2d image
                ...
                # ------- Changes end here -------
        print("THIS IS FOR IMAGE :", ind1,"-", ind2, "SUCCESSFUL")
        w=len(data)
        l1=len(data[0])
        ...
        f_img = Image.fromarray(final)
        # f_img.show()
        # print("Original size", w," x ",l1)
        # print(len(ESIHEoutput) ," x ", len(ESIHEoutput[0]))
        # print(ESIHEoutput[40][40])
